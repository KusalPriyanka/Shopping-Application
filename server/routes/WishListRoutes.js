...
router.put("/UpdateWishList", verifyToken, (req, res) => {
  WishLists.find({ userId: req.user._id }).then((wishlist) => {
    WishLists.findById(wishlist[0]._id).then((wishlist) => {
...
*/
router.delete("/DeleteWishListItem/:id", verifyToken, async (req, res) => {
